...
async function handleRegistrationSubmit(e) {
  ...
  try {
    ...
    // Try inserting into Registrations table
    let { data, error } = await supabase
      .from(TABLE_NAME)
      .insert([payload]);

    // Fallback if table name is lowercase in database
    if (error && error.message && error.message.includes('relation "Registrations" does not exist')) {
